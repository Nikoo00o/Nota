...
void main() {
  ...
  group("transfer notes use case: ", () {
    ...
    test("download should replace local changes", () async {
      final ClientAccount account = await _loginAndCreateNote();
      ...
      await sl<StoreNoteEncrypted>().call(ChangeNoteEncryptedParams(
        noteId: 1,
        decryptedName: "new",
        decryptedContent: NoteContent.saveFile(decryptedContent: utf8.encode("new"), noteType: NoteType.RAW_TEXT),
      ));

      final DateTime clientDate = DateTime.now().subtract(const Duration(minutes: 5)); // manipulate client side
      account.noteInfoList[0] = account.noteInfoList[0].copyWith(newLastEdited: clientDate);

      await sl<TransferNotes>().call(const NoParams()); //next transfer to download note again
      ...
      expect(account.noteInfoList.first.lastEdited.isAfter(clientDate), true, reason: "should have the mew time stamp");

      final List<int> bytes2 = await loadNoteBytes(noteId: 1, noteType: NoteType.RAW_TEXT);
      expect(utf8.decode(bytes2), "test", reason: "should have the old note bytes");
    });

    test("download with only name changed should not download content", () async {
      final ClientAccount account = await _loginAndCreateNote();
      ...
      final List<int> bytes2 = await loadNoteBytes(noteId: 1, noteType: NoteType.RAW_TEXT);
      expect(utf8.decode(bytes2), "test", reason: "should have the old note bytes");
      expect(localDataSourceMock.lastFileChange.isBefore(between), true, reason: "changed should be before");
    });
