...
sealed class NoteContent {
  /// The raw decrypted bytes of the note file (the full content with no offsets)!
  ...
  /// Used internally by [NoteContent.loadFile] to initialize the [bytes] as a reference (for which [headerSize] and
  /// [version] will be loaded from)
  NoteContent._load(this.bytes) {
    _checkBytes();
    _checkBaseHeaderFields();
  }

  void _checkBytes() {
    if (bytes.length < baseNoteContentHeaderSize) {
  ...
  void _checkBaseHeaderFields() {
    ...
    if (headerSize > bytes.length || headerSize < baseNoteContentHeaderSize) {
      Logger.error("header size $headerSize is bigger than ${bytes.length}, or less than $baseNoteContentHeaderSize");
      throw const FileException(message: ErrorCodes.INVALID_PARAMS);
    }
  }

  /// For byte manipulation of the [bytes] a direct access to the byte buffer
  ByteData get _data => bytes.buffer.asByteData();

  bool get isEmpty => bytes.isEmpty;

  bool get isNotEmpty => bytes.isEmpty == false;
  ...
  /// This is called when loading decrypted [bytes] from a note file. It will reference those bytes internally as a
...
final class NoteContentRawText extends NoteContent {
  ...
  ///
  /// This also calls [NoteContentRawText._save]
  ...
  void _checkRawTextHeaderFields() {
    ...
    if (bytes.length < combined) {
      Logger.error("bytes length ${bytes.length} is smaller than header size and text size $combined");
      throw const FileException(message: ErrorCodes.INVALID_PARAMS);
    }
  }
  ...
  /// app.
  /// This will create the [bytes] list with the header information and copies the [decryptedContent] into it
  factory NoteContentRawText.save({required List<int> decryptedContent}) {
    ...
    return NoteContentRawText._save(
        bytes: bytes, headerSize: headerSize, textSize: textSize, decryptedContent: decryptedContent);
  }
  ...
  ///
  /// It only returns a reference to the part of the [bytes]!
  Uint8List get text {
    final int start = headerSize;
    final int end = start + textSize;
    if (end > bytes.length) {
      Logger.error("the end of the text part $end would be bigger than the bytes ${bytes.length}");
      throw const FileException(message: ErrorCodes.INVALID_PARAMS);
    }
